...
Init_Validation()
{
  ...
  then
    echo
    echo -e ${RED} "USAGE : bash `basename $0` <Target Postgres Data Directory> "
...
Move_PG_Data_Directory_To_New_Location()
{
  ...
  # create soft link to new PG data Directory
  ln -s ${TRG_PG_DATA_DIR}/data
